...
**Every command in this course runs inside a Python environment named `demo`.** An environment is a private, isolated copy of Python and its packages, kept in its own folder.
...
**Every command in this handbook runs inside a Python environment named `demo`.** An environment is a private, isolated copy of Python and its packages, kept in its own folder.
...
| Command style | `python -m venv demo` | `conda create -n demo` |
...
```powershell
mkdir C:\Users\%USERNAME%\mlai-demo
```

```powershell
cd C:\Users\%USERNAME%\mlai-demo
```
...
This creates a `demo` folder containing your private Python.
...
Your prompt should now start with `(demo)`.
...
```bash
mkdir -p ~/mlai-demo && cd ~/mlai-demo
```
...
> ⚠️ **`error: externally-managed-environment`** when you `pip install` means you are **outside** the environment and pip is protecting your system Python. Run `source demo/bin/activate` first. Never use `--break-system-packages` to get around this.
...
> ⚠️ **Choose the right build.** On an M1/M2/M3/M4 Mac take the **Apple Silicon (arm64)** installer, not the Intel one. Check your hardware with `uname -m` — `arm64` means Apple Silicon. A Rosetta (x86_64) conda installs fine and runs almost everything, but caps PyTorch at 2.2.2, which breaks the Module 5 Hugging Face examples.
...
> ⚠️ **Before you type anything: does your prompt show `(demo)`?**
>
> ```text
> (demo) C:\Users\you\mlai-demo>     ← Windows, correct
> (demo) you@ubuntu:~/mlai-demo$     ← Ubuntu, correct
> you@ubuntu:~/mlai-demo$             ← WRONG - activate first
> ```
>
> If `(demo)` is missing, the packages install somewhere else and nothing will work. This is the most common problem in the whole course.
...
## 3.4 Install the Module 4–5 packages (demo and web apps)

```bash
pip install google-demo streamlit pillow python-dotenv
```

| Package | What it does | First used |
|---|---|---|
| `google-demo` | Calls the Gemini API | Module 4 |
...
try:
    ...
    print(f"OK    {'google-demo':<12} {'installed':<10} Module 4 - Gemini API")
except ImportError:
    print(f"FAIL  {'google-demo':<12} {'-':<10} Module 4 - Gemini API")
    missing.append("google-demo")
...
Running : /home/you/mlai-demo/demo/bin/python
-------------------------------------------------------
...
OK    google-demo installed  Module 4 - Gemini API
-------------------------------------------------------
All set. You are ready for Module 1.
```

**Look at the `Running :` line.** The path must contain `demo`. If it does not, you are running the wrong Python — your environment is not active.

---

# Step 5: Register the environment with Jupyter

So `demo` appears as a kernel in Jupyter and in VS Code notebooks:
...
Inside a notebook choose **Kernel → Change kernel → Python (demo)**.
...
3. Choose the one whose path contains **`demo`**.
...
!pip install -q -U google-demo
```

Colab already includes: `numpy`, `pandas`, `matplotlib`, `seaborn`, `scikit-learn`, `joblib`.
You must install: `google-demo`, and `streamlit` if you want to inspect app code.
...
| Task | venv | conda |
|---|---|---|
| **Activate** (Windows) | `.\demo\Scripts\Activate.ps1` | `conda activate demo` |
| **Activate** (Ubuntu/macOS) | `source demo/bin/activate` | `conda activate demo` |
...
| **Delete and start over** | delete the `demo` folder | `conda env remove -n demo` |
...
- Environment activates, and the prompt shows `(demo)`
...
- VS Code interpreter set to `demo`
